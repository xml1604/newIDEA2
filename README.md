...
前端组的界面做不到要求，想要一种类似课程表的结构。目前正在解决在servlet.java里，用个print函数显示结果。
组长要求前端组先看看servlet吧，那样就明白参数是怎么从jsp传进Servlet.，只要你在jsp的框里写了数据，再调用servlet就行了。

XIE:正在阅读流行的框架，准备进行上云部署以及优化~
